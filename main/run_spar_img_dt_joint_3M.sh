...
cd "$WORKDIR"
RUNDIR="$WORKDIR/main/spar_img_dt_joint_3M"
...
CMD=(
  ...
  --obs_type image
  --reward_scale 1.0
  --decision_timing joint --dwell_frames 4 --actionable_statuses 512,514,520 --max_skip_frames 90
  --gamma 0.94 --gae_lambda 0.95
)
echo "=== SHORT SPAR TEST: obs=image, reward_scale=1.0 (UNSCALED + decision_timing JOINT dwell4), 3M steps ==="
