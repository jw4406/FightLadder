...
from FightLadder.main.common.league import PSROLeague
...
def league_constructor(args, side, state, file=None, log_name=None, single_env=False):
    num_env = 1 if single_env else args.num_env
    env = [make_env(sf_game, state=state, side=args.side, reset_type=args.reset, rendering=args.render, enable_combo=True, null_combo=args.null_combo, transform_action=True, seed=i) for i in range(num_env)]
    ...
    if file:
    #     print(file)
    #     # load_kwargs = torch.load(file, map_location=torch.device('cpu'))["kwargs"]
    #     # load_weights = load_kwargs["agent_dict"]
    #     # model = torch.load(file)
    #     # model.set_parameters_2p(file, file)
        load_kwargs = torch.load(file, map_location=torch.device('cpu'))["kwargs"]
        load_weights = load_kwargs["agent_dict"]
        model.set_parameters(load_weights)
    return model

def psro_constructor(args, side, state, file=None, log_name=None, single_env=False):
    num_env = 1 if single_env else args.num_env
    env = [make_env(sf_game, state=state, side=args.side, reset_type=args.reset, rendering=args.render, enable_combo=True, null_combo=args.null_combo, transform_action=True, seed=i) for i in range(num_env)]
    env = VecTransposeImage2P(SubprocVecEnv2P(env))
    model = LeaguePPO(
...
def ippo_constructor(args, state, side, file=None, log_name=None, single_env=False):
    if file == None:
    ...
    def finetune_model_generator(model_file=None, lr_schedule=linear_schedule(5.0e-5, 2.5e-6), clip_range_schedule=linear_schedule(0.075, 0.025)):
    ...
    model = finetune_model_generator(model_file=file, lr_schedule=linear_schedule(2.5e-4, 2.5e-6), clip_range_schedule=linear_schedule(0.15, 0.025))
    return model
    ...
    for j in range(1, args.num_episodes + 1):
        ...
        while not done:
            if model.use_mirror is True:
                ...
                (action_other, _), (_, _states_other) = exploiter_model.predict(obs, env_index, deterministic=False)
        ...
        if info['enemy_hp'] < info['agent_hp']:
            ...
            rating1, rating2 = elo_rating(rating1, rating2, d=0)
...
def main(PLAYER):
    ...
    #finetune_model = ippo_constructor(args, side="right", file="/home/jw4406/codebase/FightLadder/main/benchmark_models/right/FSP0.pt")
    state_list = ['two_player/Guile_left/Champion.Level1.GuileVsRyu.2Player.state']
    #finetune_model = ippo_constructor(args, side="right", state=state_list[0],
    #                                  file="/home/jw4406/codebase/FightLadder/main/benchmark_models/right/FSP0.pt")
    #finetune_model = league_constructor(args, "right", state=state_list[0], file = "/home/jw4406/codebase/FightLadder/main/"
    #                                                                                   "benchmark_models/right/LEAGUE.pt")
    #finetune_model = psro_constructor(args, "right", state=state_list[0], file="/home/jw4406/codebase/FightLadder/main/"
    #                                                                               "benchmark_models/right/FSP0.pt")
    '''
    x = LeaguePPO(
        "left",
        "CnnPolicy",
    ...
    data, params, pytorch_variables = load_from_zip_file("/home/jw4406/codebase/FightLadder/main/trained_models/ppo_Guile_sa.zip")

    left_agent.set_parameters(params, exact_match=False, device=finetune_model.device)

    right_agent = left_agent
    ...
    if not EVAL:
        ...
        for i in range(len(state_list)):
            # global STATE
            # STATE = state_list[i]
            results = evaluate_sa(state_list[i], args, left_agent, right_agent, i, record=False)
        print(results)
