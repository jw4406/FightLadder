...
class BaseAlgorithm(ABC):
    ...
    def _setup_lr_schedule(self) -> None:
        ...
        for i in range(len(self.lr_schedule)):
            ...
            if hasattr(self, "learning_rate_decay_phase"):
                if (isinstance(self, MAGICS_AL)) and i >= len(self.learning_rate_decay_phase):
                    continue
                self.lr_schedule_decay[i] = get_schedule_fn(self.learning_rate_decay_phase[i])
        if len(self.lr_schedule) == 1: # single agent, we shouldn't wrap in a []
            self.lr_schedule = self.lr_schedule[0]
        print("hello")
    ...
    def _update_learning_rate(self, optimizers: Union[List[th.optim.Optimizer], th.optim.Optimizer]) -> None:
        ...
        if not isinstance(optimizers, list):
            optimizers = [optimizers]
        count = 0
        if isinstance(self, RARL_PPO):
            # we only update one at a time because this is alternating optimization!

            assert len(optimizers) == 1

            if self.update_lr_ctrl is True:
                update_learning_rate(optimizers[0], self.lr_schedule[1](self._current_progress_remaining))
            elif self.update_lr_dstb is True:
                update_learning_rate(optimizers[0], self.lr_schedule[2](self._current_progress_remaining))
            elif self.update_lr_critic is True:
                update_learning_rate(optimizers[0], self.lr_schedule[0](self._current_progress_remaining))
            else:
                raise ValueError
        ...
        if isinstance(self, (MAGICS_AL, TSS_PPO, MAGICS_PPO)):
            print("testing")
            if isinstance(self, (MAGICS_AL, TSS_PPO, MAGICS_PPO)):
                explore = 9_000_000  # heuristic
        ...
        if isinstance(self, RARL_PPO):
            ...
            if self.update_lr_ctrl is True:
                update_learning_rate(optimizer, self.lr_schedule[0](self._current_progress_remaining))
            elif self.update_lr_dstb is True:
                update_learning_rate(optimizer, self.lr_schedule[1](self._current_progress_remaining))
            elif self.update_lr_critic is True:
                update_learning_rate(optimizer, self.lr_schedule[2](self._current_progress_remaining))
            else:
                raise ValueError
